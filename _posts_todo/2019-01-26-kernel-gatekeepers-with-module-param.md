---
layout: post
title: "Abusing module_param to test kernels in production more confidently"
---

Kernel changes that simply fix bugs are generally fairly safe, and testable in
isolation. More complicated are improvements to core kernel behaviour -- those
that may have edge cases in production which are otherwise difficult to isolate
into a test case.

I recently encountered this problem while rolling out [my changes to page
reclaim behaviour to proportion it based on protection
overage](https://marc.info/?l=linux-mm&m=154829430827982&w=2). This change
fundamentally alters the way that the kernel handles memory lifecycles, and is
hard to test all edge cases for -- for example, whether such a change makes
systems more susceptible to having reclaim progress arrested, leading to OOM.
While you can devise an array of synthetic tests (and indeed in this case many
small-scale production and synthetic tests were run), there's no better test
than seeing the code succeed at production in tests at scale on a variety of
machine types.

This poses a problem, because you don't really want to have to roll back the to
the old kernel quickly in an emergency if it turns out your code unexpectedly
causes problems, potentially reducing capacity further as machines have to
reboot or are kexeced, and damaging trust with the machine owner.

At Facebook we already have a few core ways to decoupling code execution from
code rollout: [gatekeepers and
sitevars](http://sigops.org/s/conferences/sosp/2015/current/2015-Monterey/008-tang-online.pdf).
In the case of a sitevar, we take a Hack object that has its state existing
outside of the deployed code and use it in the existing code to change code
flows, behaviours, and configurations.

My immediate reaction upon noticing this problem in the kernel land was to
reach for a temporary sysctl, but this is not really ideal for a few reasons:
